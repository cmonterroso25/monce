  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#2C3E50]">Leads</h1>
        <div className="flex items-center gap-2">
          <div className="flex overflow-hidden rounded border border-slate-300">
            <Link
              href={`/dashboard/leads?vista=kanban${sufijo}`}
              className={`flex items-center gap-1 px-3 py-2 text-sm ${vista === 'kanban' ? 'bg-[#2C3E50] text-white' : 'bg-white text-slate-600'}`}
            >
              <LayoutGrid size={15} /> Kanban
            </Link>
            <Link
              href={`/dashboard/leads?vista=tabla${sufijo}`}
              className={`flex items-center gap-1 px-3 py-2 text-sm ${vista === 'tabla' ? 'bg-[#2C3E50] text-white' : 'bg-white text-slate-600'}`}
            >
              <List size={15} /> Tabla
            </Link>
          </div>
