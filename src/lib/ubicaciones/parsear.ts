...
function extraerCoordenadasGoogleMaps(url: string): { lat: number; lng: number } | null {
  ...
  if (m) return { lat: parseFloat(m[1]), lng: parseFloat(m[2]) }
  // Formato de búsqueda por coordenadas: /maps/search/LAT,LNG (el "+"
  // antes del signo negativo es el espacio codificado que Google agrega
  // entre la coma y el número negativo al compartir "esta ubicación" desde
  // el buscador de Maps, en vez de soltar un pin con @ o compartir un q=).
